...
private:
    ...
    bool initRegistration(const std::string& method, const int32_t device, const double maxDepth)
    {
        ...
        if (!depthRegLowRes->init(cameraMatrixLowRes, sizeLowRes, cameraMatrixDepth, sizeIr, distortionDepth, rotation, translation, 0.5f, maxDepth, device) || !depthRegHighRes->init(cameraMatrixColor, sizeColor, cameraMatrixDepth, sizeIr, distortionDepth, rotation, translation, 0.5f, maxDepth, device)) {
            delete depthRegLowRes;
            delete depthRegHighRes;
            return false;
        }
    ...
    void initCalibration(const std::string& calib_path, const std::string& sensor)
    {
        ...
        OUT_DEBUG("camera matrix color:" FG_CYAN << std::endl
                                                 << cameraMatrixColor << NO_COLOR);
        OUT_DEBUG("distortion coefficients color:" FG_CYAN << std::endl
                                                           << distortionColor << NO_COLOR);
        OUT_DEBUG("camera matrix ir:" FG_CYAN << std::endl
                                              << cameraMatrixIr << NO_COLOR);
        OUT_DEBUG("distortion coefficients ir:" FG_CYAN << std::endl
                                                        << distortionIr << NO_COLOR);
        OUT_DEBUG("camera matrix depth:" FG_CYAN << std::endl
                                                 << cameraMatrixDepth << NO_COLOR);
        OUT_DEBUG("distortion coefficients depth:" FG_CYAN << std::endl
                                                           << distortionDepth << NO_COLOR);
        OUT_DEBUG("rotation:" FG_CYAN << std::endl
                                      << rotation << NO_COLOR);
        OUT_DEBUG("translation:" FG_CYAN << std::endl
                                         << translation << NO_COLOR);
        OUT_DEBUG("depth shift:" FG_CYAN << std::endl
                                         << depthShift << NO_COLOR);
    }
    ...
    void receiveColor()
    {
        ...
        if (status[COLOR_HD] || status[COLOR_HD_RECT] || status[COLOR_QHD] || status[COLOR_QHD_RECT] || status[MONO_HD] || status[MONO_HD_RECT] || status[MONO_QHD] || status[MONO_QHD_RECT]) {
            cv::Mat tmp;
            cv::flip(color, tmp, 1);
